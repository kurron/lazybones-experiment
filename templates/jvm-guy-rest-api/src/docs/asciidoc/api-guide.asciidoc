...
Magni tries to adhere as closely as possible to standard HTTP and REST conventions in its
use of HTTP verbs.
...
| X-Correlation-Id
| Magni is part of a large network of cooperating services and publishes its log messages to a centralized server.  To facilitate the tracing of events and messages between systems, Magni requires that a correlation id be provided with each request. If the client is the initiator of an action, then a new id should be generated.  If the client, however, is reacting to an outside event or message, the client should propogate the correlation id it was handed.
|===
...
Magni uses a simple hypermedia control that includes links to other resources that the client
...
Magni does not currently require any form of authentication, expecting that the service will not be
exposed directly to the public internet.  Magni does not currently rate limit requests, expecting that
...
both the asset's actual type and additional meta-data.  Magni adheres to the https://tools.ietf.org/html/rfc2045[RFC 2045]
...
| Required. The number of minutes that Magni should hold onto the asset before evicting it from the system.
| Content-Type
...
well as the MIME type of the Magni control.  In event of a failure, the control will be
